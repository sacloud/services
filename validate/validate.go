...
type Validator struct {
	...
	FormatNameFunc     func(name string) string
...
func New(service services.Service) *Validator {
	...
	if config != nil {
		// option
		for _, opt := range config.OptionDefs {
			v.RegisterCollectionValidator(opt.Key, opt.GetValues())
		}

		if config.FormatErrorFieldNameFn != nil {
			v.FormatNameFunc = config.FormatErrorFieldNameFn
		}
	}
...
func (v *Validator) init() {
	v.initOnce.Do(func() {
		...
		v.instance.RegisterTagNameFunc(func(fld reflect.StructField) string {
			tags := []string{"meta", "name", "yaml", "json"}
...
// RegisterCollectionValidator 要素/コレクションを示すキーに対しそれぞれoneofエイリアスを登録する
func (v *Validator) RegisterCollectionValidator(key string, allowedValues []string) {
	v.init()

	v.instance.RegisterAlias(key, fmt.Sprintf("oneof=%s", strings.Join(allowedValues, " ")))
}
...
func (v *Validator) errorFromValidationErr(target interface{}, err validator.FieldError) error {
	...
	if v.FormatNameFunc != nil {
		actualName = v.FormatNameFunc(actualName)
	}
